...
pg_logical_get_remote_repset_tables(PGconn *conn, List *replication_sets)
{
	...
	if (pglogical_remote_function_exists(conn, "pglogical", "show_repset_table_info_by_target", 3))
	{
		/* PGLogical 2.3+ */
	...
	else if (pglogical_remote_function_exists(conn, "pglogical", "show_repset_table_info", 2))
	{
		/* PGLogical 2.0+ */
		appendStringInfo(&query,
						 "SELECT i.relid, i.nspname, i.relname, i.att_list,"
								   ...
								   List *replication_sets)
{
	...
	remoterel->nsptarget = pstrdup(PQgetvalue(res, 0, 5));
	remoterel->reltarget = pstrdup(PQgetvalue(res, 0, 6));
...
pg_logical_get_remote_repset_sequences(PGconn *conn, List *replication_sets)
{
	...
	appendStringInfo(&query,
			 "select set_nsptarget, set_seqtarget FROM pglogical.replication_set_seq r, "
